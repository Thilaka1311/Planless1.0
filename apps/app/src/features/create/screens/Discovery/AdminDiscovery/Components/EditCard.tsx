...
export const EditCard: React.FC<EditCardProps> = ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    console.log("[EditCard] handleSave called. Target Item ID:", item.id);
    console.log("[EditCard] Auth token exists:", !!token, "Length:", token?.length || 0);
    try {
      await adminUpdateItem(item.id, formData, config, token);
      console.log("[EditCard] Save request completed successfully");
    ...
    >
      ...
      <div className="shrink-0 px-5 pt-safe-top pt-4 pb-3 flex items-center gap-4 border-b border-white/[0.06]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] font-medium text-[#71717A] tracking-widest uppercase mt-0.5">
            Admin CMS · {config.title}
          </p>
        </div>
      </div>
