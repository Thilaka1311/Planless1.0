...
export const DiningCard: React.FC<CardProps> = ({ item, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className="w-[240px] h-[160px] shrink-0 rounded-2xl relative overflow-hidden bg-zinc-950 border border-zinc-900/80 shadow-lg flex flex-col justify-between p-3.5 cursor-pointer hover:border-orange-500/30 hover:shadow-[0_0_15px_rgba(249,115,22,0.15)] transition-all duration-300 group select-none"
    >
      ...
      <div className="z-10 flex justify-end">
        <span className="bg-orange-500/15 text-orange-400 border border-orange-500/30 px-2 py-0.5 rounded-full font-mono text-[9px] flex items-center gap-0.5 font-bold shadow-sm backdrop-blur-sm">
          <Star className="w-2.5 h-2.5 fill-current text-orange-400" />
          {rating}
        </span>
      </div>
      ...
      <div className="z-10 text-left space-y-0.5">
        ...
        <div className="flex items-baseline justify-between gap-2">
          <p className="text-[9px] text-zinc-400 font-sans truncate max-w-[130px]">
            {item.description || "Cafe & Desserts"}
          </p>
          <span className="text-[8px] font-mono text-orange-400 font-bold uppercase tracking-wider shrink-0">
            {distance}
          </span>
        </div>
      </div>
